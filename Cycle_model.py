...
from layer import ReflectionPadding2D
...
from collections import deque # Stack, Queue 기능을 동시에 수행할 수 있는 deque 모듈
...
class CycleGAN():
    def __init__(self
                 ...
                 , gen_n_filters
                 , disc_n_filters
                 , buffer_max_length=50 # default params
                 ):
        ...
        patch_r = int(self.img_rows / 2 ** 2)
        patch_c = int(self.img_cols / 2 ** 2)
    ...
    def build_generator_resnet(self):
        ...
        def conv7s1(layer_input, filters, final):
            ...
            y = Conv2D(filters, kernel_size=(7, 7), strides=1, padding='valid', kernel_initializer=self.weight_init)(y) # padding valid: 사용하지 않음
            if final:
                y = Activation('tanh')(y)
            else:
        ...
        def residual(layer_input, filters): # 잔차 블록
        ...
        y = conv7s1(y, 4, True)     # channel 맞춰주기
        output = y
    ...
    def build_discriminator(self):
        ...
        y = conv4(y, self.disc_n_filters * 4, stride=1) # 원래 2에서 1로 수정 > shape 맞춰줘야
    ...
    def train(self, data_loader, run_folder, epochs, test_A_file, test_B_file, batch_size=1, sample_interval=50):
        ...
        valid = np.ones((batch_size,) + self.disc_patch) # (1, 20, 20, 1) 로 설정
        ...
        for p in range(2):
            ...
            for i in range(r):
                for j in range(c):
                    ...
                    axs[i, j].axis('off')
                    cnt += 1
            fig.savefig(run_folder+"images/%d_%d_%d.png" % (p, self.epoch, batch_i))
            plt.close()
